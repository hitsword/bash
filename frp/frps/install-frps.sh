...
str_install_shell=https://soft.huayizhiyun.com/manage/frp/bash/frps/install-frps.sh
shell_update(){
    fun_huayi "clear"
    ...
    if [ -s ${str_program_dir}/${program_name} ] && [ -s ${program_init} ]; then
        ...
        case "${str_log_file}" in
            1|[yY]|[yY][eE][sS]|[oO][nN]|[tT][rR][uU][eE]|[eE][nN][aA][bB][lL][eE])
                str_log_file=$str_program_dir"/frps.log"
        ...
        echo "#####################################################"
        read -p "Enter your choice (1, 2 or exit. default [2]): " str_kcp
        case "${str_kcp}" in
            1|[yY]|[yY][eE][sS]|[oO][nN]|[tT][rR][uU][eE]|[eE][nN][aA][bB][lL][eE])
                set_kcp="true"
                ;;
...
# console or real logFile path like $str_program_dir/frps.log
...
uninstall)
    ...
    ;;
update)
    update_program_server_huayi 2>&1 | tee /root/${program_name}-update.log
    ;;
*)
    fun_huayi
